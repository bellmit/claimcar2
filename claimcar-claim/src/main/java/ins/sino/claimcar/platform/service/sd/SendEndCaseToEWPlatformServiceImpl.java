...
import ins.sino.claimcar.trafficplatform.vo.EWEndCaseBasePartVo;
import ins.sino.claimcar.trafficplatform.vo.EWEndCaseClaimCoverDataVo;
import ins.sino.claimcar.trafficplatform.vo.EWEndCaseRequest;
import ins.sino.claimcar.trafficplatform.vo.EWEndCaseRequestBody;
import ins.sino.claimcar.trafficplatform.vo.EWEndCaseResponse;
...
@Path("sendEndCaseToEWPlatformService")
public class SendEndCaseToEWPlatformServiceImpl implements SendEndCaseToEWPlatformService {

	private Logger logger = LoggerFactory.getLogger(SendEndCaseToEWPlatformServiceImpl.class);
	...
	public void SendEndCaseToEWPlatform(PrpLEndCaseVo endVo) {
		...
		if(!StringUtils.isBlank(endVo.getCompensateNo())&& !"CompeWfZero".endsWith(endVo.getCompensateNo())){
			compensateVo = compensateService.findCompByPK(endVo.getCompensateNo());
		}
		...
		try{
			if(compensateVo!=null){
				...
				if(cMainVo.getComCode().startsWith("62")){// 山东机构
					...
					try{
				requestXML = getRequestXML(endVo,compensateVo);
				logger.info("=================结案送山东预警请求报文"+requestXML);
				responseXML = earlyWarnService.requestSDEW(requestXML, urlStr);
				logger.info("=================结案送山东预警返回报文"+responseXML);
				if(responseXML!=null && !"".equals(responseXML)){
					EWEndCaseResponse responseVo = ClaimBaseCoder.xmlToObj(responseXML, EWEndCaseResponse.class);
			...
			e.printStackTrace();
			logger.info("==========结案送山东预警报错=========="+responseXML);
			logVo.setStatus("0");
			...
			if(compensateVo !=null){
				...
				logVo.setBusinessType(BusinessType.SDEW_endCase.toString());
	            logVo.setBusinessName(BusinessType.SDEW_endCase.getName());
				...
				}else if(cMainVo.getComCode().startsWith("10")){// 广东机构
					String urlStr = SpringProperties.getProperty("GDWARN_URL");// 获取请求地址
					try{
						requestXML = getRequestXML(endVo,compensateVo);
						logger.info("=================结案送广东平台请求报文"+requestXML);
						responseXML = earlyWarnService.requestSDEW(requestXML,urlStr);
						logger.info("=================结案送广东平台返回报文"+responseXML);
						if(responseXML!=null&& !"".equals(responseXML)){
							EWEndCaseResponse responseVo = ClaimBaseCoder.xmlToObj(responseXML,EWEndCaseResponse.class);
							if("1".equals(responseVo.getHead().getResponseCode())){
								logVo.setStatus("1");
						...
						e.printStackTrace();
						logger.info("==========结案送广东平台报错=========="+responseXML);
						logVo.setStatus("0");
						logVo.setErrorMessage(e.getMessage());
					}
					finally{
						if(compensateVo!=null){
							...
							logVo.setResponseXml(responseXML);
							logVo.setRegistNo(compensateVo.getRegistNo());
							logVo.setBusinessType(BusinessType.GDEW_endCase.toString());
							logVo.setBusinessName(BusinessType.GDEW_endCase.getName());
							logVo.setOperateNode(FlowNode.EndCas.name());
							logVo.setRequestTime(new Date());
							logVo.setCreateTime(new Date());
							logVo.setCreateUser(compensateVo.getCreateUser());
				...
				}else if(cMainVo.getComCode().startsWith("50")){// 河南机构
					String urlStr = SpringProperties.getProperty("HNWARN_URL");// 获取请求地址
					try{
						requestXML = getRequestXML(endVo,compensateVo);
						logger.info("=================结案送河南平台请求报文"+requestXML);
						responseXML = earlyWarnService.requestSDEW(requestXML,urlStr);
						logger.info("=================结案送河南平台返回报文"+responseXML);
						if(responseXML!=null&& !"".equals(responseXML)){
							EWEndCaseResponse responseVo = ClaimBaseCoder.xmlToObj(responseXML,EWEndCaseResponse.class);
							if("1".equals(responseVo.getHead().getResponseCode())){
								logVo.setStatus("1");
						...
						e.printStackTrace();
						logger.info("==========结案送河南平台报错=========="+responseXML);
						logVo.setStatus("0");
						logVo.setErrorMessage(e.getMessage());
					}
					finally{
						if(compensateVo!=null){
							...
							logVo.setResponseXml(responseXML);
							logVo.setRegistNo(compensateVo.getRegistNo());
							logVo.setBusinessType(BusinessType.HNEW_endCase.toString());
							logVo.setBusinessName(BusinessType.HNEW_endCase.getName());
							logVo.setOperateNode(FlowNode.EndCas.name());
							logVo.setRequestTime(new Date());
							logVo.setCreateTime(new Date());
							logVo.setCreateUser(compensateVo.getCreateUser());
	 ...
	 * @return
	 */
	public String getRequestXML(PrpLEndCaseVo endVo,PrpLCompensateVo compensateVo){
		String requestXML = "";
		EWEndCaseRequest request = new EWEndCaseRequest();
		EWReqHead head = new EWReqHead();
		EWEndCaseRequestBody body = new EWEndCaseRequestBody();
		EWEndCaseBasePartVo basePart = new EWEndCaseBasePartVo();
		List<EWEndCaseClaimCoverDataVo> claimCoverDataVoList = new ArrayList<EWEndCaseClaimCoverDataVo>();
		...
		if(cMainVo.getComCode().startsWith("62")){// 山东机构
			...
			head.setRequestType(BusinessType.SDEW_endCase.getCode());
			head.setUser(user);
			...
			head.setRequestType(BusinessType.GDEW_endCase.getCode());
			head.setUser(user);
			head.setPassWord(passWord);
			...
			head.setRequestType(BusinessType.HNEW_endCase.getCode());
			head.setUser(user);
			head.setPassWord(passWord);
		}
		PrpLCheckDutyVo checkDutyVo = checkTaskService.findCheckDuty(endVo.getRegistNo(),1);
		PrpLPadPayMainVo padPayMainVo = padPayPubService.findPadPay(endVo.getRegistNo());
		PrpLCertifyMainVo certifyMainVo = certifyPubService.findPrpLCertifyMainVoByRegistNo(endVo.getRegistNo());
		List<PrpLDlossCarMainVo> lossCarMainVos = lossCarService.findLossCarMainBySerialNo(endVo.getRegistNo(),1);
		...
		basePart.setClaimAmount(compensateVo.getSumAmt());//TODO 确认传本次的还是历次的总和
		String claimCloseTime = sdf.format(endVo.getEndCaseDate()).trim();
		claimCloseTime = claimCloseTime.replaceAll("-","");
		claimCloseTime = claimCloseTime.replaceAll(" ","");
		claimCloseTime = claimCloseTime.replaceAll(":","");
		basePart.setClaimCloseTime(claimCloseTime);//结案时间 精确到分
		basePart.setIsInsured("1");//是否属于保险责任，默认为是
		String claimType = "1";//正常理赔
		if(DutyType.CIINDEMDUTY_N.equals(checkDutyVo.getCiDutyFlag())){
			claimType = "2";//无责赔案
		}
		...
		basePart.setPayCause("");
		if("1101".equals(compensateVo.getRiskCode())){
			basePart.setIsRefuseCase(certifyMainVo.getIsJQFraud()); //是否为交强拒赔案件
		}else{
			basePart.setIsRefuseCase(certifyMainVo.getIsSYFraud()); //是否为商业拒赔案件
		}
		if("1".equals(basePart.getIsRefuseCase())){//拒赔为是才有拒赔原因
			basePart.setRefuseCause(certifyMainVo.getFraudRefuseReason());
		}else{
			basePart.setRefuseCause("");
		}
		basePart.setAccidentType("100");//交通事故
		basePart.setDirectClaimAmount(compensateVo.getSumPaidFee());//TODO 确认是历次还是本次
		basePart.setIsTotalLoss(lossCarMainVos.get(0).getIsTotalLoss());//TODO 确认是否是标的车的是否全损
		
		/**无损失0结案及冲销0结案案件由于claimCoverData必传，做特殊处理，手工生成标的车0损失记录或交强0损失记录**/
		if(BigDecimal.ZERO.compareTo(compensateVo.getSumPaidAmt())==0 
			...
			EWEndCaseClaimCoverDataVo claimCoverData = new EWEndCaseClaimCoverDataVo();
			if("1101".equals(compensateVo.getRiskCode()) ){
				...
				claimCoverData.setCoverageCode(kindCodeTranService.findTransCiCode("00", "1201", "CovergeCode", kindCode));
			}
		...
		if(compensateVo.getPrpLLossProps() != null && compensateVo.getPrpLLossProps().size() > 0){
			for(PrpLLossPropVo itemVo : compensateVo.getPrpLLossProps()){
				if(!"9".equals(itemVo.getPropType())){//不是附加险损失
					EWEndCaseClaimCoverDataVo claimCoverData = new EWEndCaseClaimCoverDataVo();
					if("1101".equals(compensateVo.getRiskCode()) ){
						claimCoverData.setCoverageCode("100");//交强险出险时，“赔偿险种代码”必须传送“100-机动车强制责任保险”
					}else{
			...
			for(PrpLLossPersonVo personVo : compensateVo.getPrpLLossPersons()){
				for(PrpLLossPersonFeeVo feeVo : personVo.getPrpLLossPersonFees()){
					...
					if("1101".equals(compensateVo.getRiskCode()) ){
						...
						//山东预警系统编码与全国车险平台1201险种编码一致
						claimCoverData.setCoverageCode(kindCodeTranService.findTransCiCode("00", "1201", "CovergeCode", personVo.getKindCode()));
					}
		...
		body.setClaimCoverData(claimCoverDataVoList);

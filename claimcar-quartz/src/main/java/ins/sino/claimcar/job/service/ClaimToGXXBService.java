...
@Service(value = "claimToGXXBService")
public class ClaimToGXXBService {
	...
	public void doJob() {
		...
		if(logListByRegist!=null && logListByRegist.size()>0){
			for(ClaimInterfaceLogVo logVo:logListByRegist){
				try{
					...
					e.printStackTrace();
					logger.info("==========报案送广西消保补传报错，报案号："+logVo.getRegistNo()+e.getMessage());
				}
		...
		if(logListByEndCase!=null && logListByEndCase.size()>0){
			for(ClaimInterfaceLogVo logVo:logListByEndCase){
				try{
					...
					e.printStackTrace();
					logger.info("==========结案送广西消保补传报错，结案号："+logVo.getCompensateNo()+e.getMessage());
				}
			}
		}
		...
		try{
			...
			e.printStackTrace();
			logger.info("==========报案送广西消保报错："+e.getMessage());
		}
		...
		try{
			...
			e.printStackTrace();
			logger.info("==========结案送广西消保报错："+e.getMessage());
		}
				...
				if(comCodeList!=null && registVo.getComCode()!=null && 
					...
					if(cMainVoList!=null && cMainVoList.size()>0){
						for(PrpLCMainVo cMainVo:cMainVoList){
							...
							caseBean.setAreaid(getAreaid(registVo.getComCode()));

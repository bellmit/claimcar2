...
(function($,win,undef){
	...
	Validform.util={
		dataType:{
			...
			"mlandline":/^((13[0-9]{9})|(14[0-9]{9})|(15[0-9]{9})|(16[0-9]{9})|(18[0-9]{9})|(17[0-9]{9})|(19[0-9]{9})|(0\d{2,3}-\d{7,8}))$/,
			"e":/^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
			"url":/^(\w+:\/\/)?\w+(\.\w+)+.*$/,
			//"idcard":/^\d{15}(\d{2}[A-Za-z0-9])?$/,
			"idcard":function(gets,obj,curform,regxp){
				return validIdentityCode(gets);
			},
			"vin":function(gets,obj,curform,regxp){
				//参数gets是获取到的表单元素值，obj为当前表单元素，curform为当前验证的表单，regxp为内置的一些正则表达式的引用;
				return validVINCode(gets);
				//注意return可以返回true 或 false 或 字符串文字，true表示验证通过，返回字符串表示验证失败，字符串作为错误提示显示，返回false则用errmsg或默认的错误提示;
			},
			"amount":/^([1-9][\d]{0,7}|0)(\.[\d]{1,2})?$/,
			"rate":/^(([1-9][0-9]|[1-9])(\.\d{1,2})?|0\.\d{1,2}|100)$/,
		},
 ...
 * 校验车架号
 * */
function validVINCode1(frameNo){
	if((frameNo.indexOf("I")>=0||frameNo.indexOf("O")>=0||frameNo.indexOf("Q")>=0)
			||frameNo.length!=17||isBlank(frameNo)){
//		alert("车架号不能含有IOQ且不能超过17位，请核实");
		return false;
	}
	return true;
}

function validVINCode(sVIN) 
{ 
	//"I")>=0||frameNo.indexOf("O")>=0||frameNo.indexOf("Q")
	...
	if (sVIN.length == 17) { 
		...
		try { 
			...
			if(blKYZF) { 
				intTemp = iJQS%11; 
				if (intTemp == 10){ //计算值为10，校验位为X
					sJYW = "X"; 
				} 
				else { 
					sJYW = intTemp.toString(); 
				} 
				if (sJYW == sVIN.substr(8, 1)){//取第9位的值，判断是否相等
					valid = true; 
				}
			} 
			else { 
...
function validIdentityCode(code) { 
	if (!checkCard(code)) {
		return false;
	}
}
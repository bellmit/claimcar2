var columns = [
  ...
  function rowCallback(row, data, displayIndex, displayIndexFull) {
	...
	var url = contextPath + "/spcEdit/delete/" + data.id;
...
$(function() {
	...
	$("button.btn-create").click(function() {
		location.href = contextPath + "/spcEdit/edit/new?editMode=create";
	...
	$("button.btn-search").click(
		function() {
			...
			ajaxList.targetUrl = contextPath + '/spcEdit/search?'
					+ $("#form").serialize();
	...
	$("button.btn-deleteAll").click(function() {
		...
		var url = contextPath + "/spcEdit/deleteAll/" + selectedIds;
	...
	$("button.btn-export").click(function() {
		...
		open(contextPath + "/spcEdit/export/" + selectedIds);
	});
	$("button.btn-import").click(function() {

		bootbox.confirm("导入会删除原有页面编号数据，确定要导入吗?", function(result) {
			if (result) { 
				showModalDialog(contextPath + "/spcEdit/showUpload/one.dialog",'','status:no;dialogRight:0;dialogTop:200px;dialogWidth:600px;resizable:yes;help:no');
			}
		});

	});
	$("button.btn-backup").click(function() {
		open(contextPath + "/spcEdit/backup");
	});
	$("button.btn-recover").click(function() {
		bootbox.confirm("恢复会删除原有页面编号数据，确定要恢复吗?", function(result) {
			if (result) { 
				showModalDialog(contextPath + "/spcEdit/showUpload/all.dialog",'','status:no;dialogRight:0;dialogTop:200px;dialogWidth:600px;resizable:yes;help:no');
			}
		});
	});

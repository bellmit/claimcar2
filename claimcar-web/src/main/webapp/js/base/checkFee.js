...
$(function() {
	...
	ajaxEdit.targetUrl = "/claimcar/checkfee/saveCheckFee.do";
	ajaxEdit.rules = null;
...
function submitNextNode(element) {
	...
	if(auditStatus == "save"){//暂存
		...
		ajaxEdit.setTargetUrl("/claimcar/checkfee/submitCheckFee.do");
		$("#fm").submit();
	}else if(auditStatus == "cancel"){//注销
		ajaxEdit.setTargetUrl("/claimcar/checkfee/submitCancel.do");
		$("#fm").submit();
	}
}

function layerShow(checkMainId){
	var flag = "look";
	index=layer.open({
	    type: 2,
	    title: '修改机构信息',
	    ...
	    skin: 'yourclass',
	    area: ['1100px', '550px'],
	    content:"/claimcar/checkagency/checkAgencyEdit?Id="+checkMainId+"&flag="+flag,
	    end:function(){
	    }
	});
}

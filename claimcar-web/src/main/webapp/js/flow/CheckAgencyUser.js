...
$(function() {
	...
	ajaxEdit.targetUrl = "/claimcar/checkagency/existCheckAgencyCode";
	ajaxEdit.afterSuccess = function(returnData) {
		...
		if($("#checkCode").val() == ''){
			layer.msg("查勘机构代码不能为空！");
		} else{
			var params = $("#IntermEditForm").serialize();
			$.ajax({
				url: "/claimcar/checkagency/saveCheckAgencyInfo",
				type : 'post',
				dataType : 'json',
				data : params,
				async : false,
				success : function(result){
					if(result.status == 500){
						...
						var prpdIntermMain = eval(result);// 接收ajax返回的IntermMainVo，用于判断是否允许编辑查勘服务页面
...
function layerShowServ(title, id) {// 弹出资费信息页面
	var checkMainID = $("#prpdCheckMainID").val();
	if (checkMainID == "") {
		layer.msg("请先保存查勘机构信息再设置资费标准！");
	} else {
		index = layer.open({
			type : 1,
			title : title,
...
function layerShowBankInfo() {
	var id = $("input[name='prpdCheckBankMainVo.Id']").val();
	var url = '/claimcar/checkagency/checkAgencyEdit_Bank?Id=' + id;
	layer.open({
		type : 2,
		area : [ '900px', '345px' ],
		fix : false, // 不固定
...
// 增加员工信息
function addCheckAgencyUserTr(checkId) {
	var selectedContent=$("#checkUserCode").val();
	var selectedItem =$("#checkUserCode").find("option[value='"+selectedContent+"']").text();
	...
	var checkSize = parseInt($intermUser_size.val(), 10);
	var params = {
		"per_index" : checkSize,
		"checkSize" : checkSize,
		"userCode" : userCode,
	...
	if(selectedContent == null || selectedContent == ""){
		...
		var url = "/claimcar/checkagency/addPersRow.ajax";
		$.post(url, params, function(result) {
			$table_cont.append(result);
			$intermUser_size.val(checkSize + 1);
		});
	}
};
function checkUserDelete(element) {
	
	var index = $(element).attr("name").split("_")[1];// 下标
	var intermUserPrefix = "prpdcheckUserVo";
...
function insertCheckName(){
	var code = $("#checkCode").val();
	var code_Name =$("#checkCode").find("option[value='"+code+"']").text();

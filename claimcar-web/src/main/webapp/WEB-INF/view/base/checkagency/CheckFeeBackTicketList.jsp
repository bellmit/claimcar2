				...
				<div class="formtable f_gray4">
					...
					<input type="hidden" name="nodeCode" value="">
						<div class="row mb-3 cl">
							...
							<div class="formControls col-3">
								<app:codeSelect codeType="" type="select" id="intermCode" name="prpLWfTaskQueryVo.checkCode" lableType="name" value="" dataSource="${dictVos}"/>
...
function rowCallback(row, data, displayIndex, displayIndexFull) {
	$('td:eq(0)', row).html("<a  onclick=checkMoreMessage('"+data.bussNo+"','"+data.accountId+"','"+data.backaccountId+"','"+data.checkCode+"','"+data.registNo+"');>"+data.bussNo+"</a>");
...
function search(){
	...
	if(isIgnoreTime(bussNo,claimNo)){
		...
		if(day>30){
			layer.alert("申请日期查询范围最大不能超过30天");
			return false;
		}
	} 
	var ajaxList = new AjaxList("#DataTable_"+handleStatus);
	ajaxList.targetUrl ='/claimcar/checkfee/checkFeeAccountSearch';
...
//本案件下，公估费费用查看yzy
var assessor_index;
function checkMoreMessage(bussNo,accountId,backaccountId,checkCode,registNo) {
	var url = "/claimcar/checkfee/checkFeeMoreMessageList?bussNo="+bussNo+"&accountId="+accountId +"&backaccountId="+backaccountId+"&checkCode="+checkCode+"&registNo="+registNo;

...
<body>
	<div class="fixedmargin page_wrap">
		<div class="table_cont">
			...
			<input type="hidden" id="auditOpinionSign" value="${auditOpinionSign}" />
				...
				<%@include file="CheckFeeMoreMessage.jsp"%>
				</div>
				</br>
			<!-- 审核意见-->
			<div id="opnion">
			<%@include file="AuditOpnionView.jsp"%>
				...
				<div class="text-c mt-10">
				    <input class="btn btn-primary" onclick="assPayCustomView('${accountId}','88','${prpDAccRollBackAccountVo.certiNo}','${prpDAccRollBackAccountVo.id}','${checkCode}','${registNo}')" id="look" type="botton" style="display:none;" value="查看银行信息" />
					<input class="btn btn-primary" id="mod" onclick="assModifiAccInfo('${accountId}','${prpDAccRollBackAccountVo.errorType}','${prpDAccRollBackAccountVo.certiNo}','${prpDAccRollBackAccountVo.id}','${checkCode}','${registNo}')" type="botton" style="display:none;"  value="修改银行信息" />
	...
	<script type="text/javascript">
		...
		function assModifiAccInfo(accountId,errorType,certiNo,backaccountId,checkCode,registNo){
			if(isBlank(accountId)){
				layer.msg("有效的银行信息Id为空,请检查该公估机构下的银行账号信息!",4000);
				return false;
			}
			paycustom_url="/claimcar/checkfee/checkFeeBankInfoQuery?accountId="+accountId+"&errorType="+errorType+"&certiNo="+certiNo+"&backaccountId="+backaccountId+"&checkCode="+checkCode+"&registNo="+registNo;
			layer.open({
				type : 2,
				title : '银行信息修改',
				shade : false,
		...
		function assPayCustomView(accountId,errorType,certiNo,backaccountId,checkCode,registNo){
			if(isBlank(accountId)){
				layer.msg("有效的银行信息Id为空,请检查该公估机构下的银行账号信息!",4000);
				return false;
			}
			paycustom_url="/claimcar/checkfee/checkBankInfoQuery?accountId="+accountId+"&errorType="+errorType+"&certiNo="+certiNo+"&backaccountId="+backaccountId+"&checkCode="+checkCode+"&registNo="+registNo;
			layer.open({
				type : 2,
				title : '银行信息查看',
				shade : false,

		...
		<table class="table table-border">
			...
			<tbody class="text-c">
				<c:forEach var="vatQueryViewVo" items="${vatQueryViewVos}" varStatus="status">
					<tr>
						<td><input type="radio" name="chooseReg" id="${status.index}_${vatQueryViewVo.billNo}_${vatQueryViewVo.billCode}_${prpLAcheckMainVo.taskId}" value="${vatQueryViewVo.vidflag}_${vatQueryViewVo.billContId}_${vatQueryViewVo.registerStatus}_${checkBankMainVo.id}"/></td>
						<c:choose>
						<c:when test="${vatQueryViewVo.registerStatus eq '1' }">
						<td><a class="btn" onclick="cheFeebillView('${vatQueryViewVo.billContId}')">${vatQueryViewVo.billNo }</a></td>
						</c:when>
						<c:otherwise>
...
<input type="text"  class="input-text" id="underWriteFlag"  name="prpLAcheckMainVo.underWriteFlag" value="${prpLAcheckMainVo.underWriteFlag}" hidden="hidden">
...
function billRegister(){
	...
	var arry1=chooseId.split("_");
	 var goUrl ="/claimcar/checkfee/cheFeeRegister.do?taskNo="+arry1[3]+"&billId="+arry[1]+"&interCodeId="+arry[3];
	 openTaskEditWin("发票(查勘费)登记",goUrl);
}
/**
 * 删除数据记录
 */
 ...
 * 跳转到发票与计算书绑定展示页面
 */
function cheFeebillView(billId){
	 var goUrl ="/claimcar/checkfee/cheFeebillView.do?billId="+billId;
	 openTaskEditWin("发票(查勘费)信息",goUrl);
}
</script>
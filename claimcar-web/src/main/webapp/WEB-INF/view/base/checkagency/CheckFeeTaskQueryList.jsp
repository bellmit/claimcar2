...
<body>
			...
			<div class="table_cont pd-10">
				<div class="formtable f_gray4">
					<form id="form" name="form" class="form-horizontal" role="form" method="post">
						...
						<div class="row mb-3 cl">
							<label class="form-label col-2 text-c">查勘机构</label>
							<div class="formControls col-4">
	...
	<script type="text/javascript">
		...
		function rowCallback(row, data, displayIndex, displayIndexFull) {
			$('td:eq(0)', row).html("<a  onclick='checkFee(" + data.taskId + ");'>" + data.registNo + "</a>");
			var handleStatus = $("input[name='handleStatus']:checked").val();
			if(handleStatus !='9'){
				$('td:eq(4)',row).html("<span class='btn-upload'><a href='javascript:void();' class='btn btn-primary radius'>上传</a><input name='file' id='Upfile"+displayIndex+"' type='file' value='点我上传'  multiple='multiple' onchange=importExcel('"+data.registNo+"','"+data.checkCode+"','"+data.checkCodeName+"','"+displayIndex+"') class='input-file'/></span>&nbsp;&nbsp;<a href=/claimcar/bill/urlReqQueryParam.do?bussNo="+data.registNo+" target='_blank' class='btn btn-primary radius'>查看</a>");
		...
		function search() {
			...
			ajaxList.targetUrl = '/claimcar/checkfee/checkFeeTaskQuery.do';// + $("#form").serialize();
       ...
       function importExcel(registNo,checkCode,checkCodeName,displayIndex) {
	        ...
	        formData.append("codeName",checkCodeName);
	        formData.append("taskType","C");
	        $.ajax({

...
<body>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			if (infomsg == "1") {
				layer.alert("Excel导入成功！", function(){
			...
			/* 	var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
				...
				layer.alert("Excel导入失败！", function(){
					var index = parent.layer.getFrameIndex(window.name); 
						parent.layer.close(index);
				});
			} else if(infomsg == "3"){
				layer.alert("Excel导入失败请检查导入的数据正确！", function(){
					var index = parent.layer.getFrameIndex(window.name); 
						parent.layer.close(index); 
				});
			} else if(infomsg =="4"){
				...
				layer.alert("您导入了一个空文件，请重新导入excel文件！", function(){
					var index= parent.layer.getFrameIndex(window.name);
					parent.layer.close(index); 
				});
			} else if(infomsg =="7"){
				layer.alert("您导入的不是excel文件，请导入excel文件！", function(){
					var index= parent.layer.getFrameIndex(window.name);
					parent.layer.close(index); 
				});
			}	

...
<body>
					...
					<input type="hidden"  name="subNodeCode"  value="${subNodeCode }" > --%>
						<div class="row mb-3 cl">
							...
							<div class="formControls col-3">
								<input type="text" class="input-text" name="platformQueryVo.registNo" value="" />
							</div>
	...
	$(function() {
		...
		function rowCallback(row, data, displayIndex, displayIndexFull) {
			if (data.status == "0") {
				$('td:eq(3)', row).html("上传失败");
			} else if (data.status == "1")  {
				$('td:eq(3)', row).html("上传成功");
			}else if (data.status == "2")  {
				$('td:eq(3)', row).html("失效");
			}else if (data.status == "3")  {
				$('td:eq(3)', row).html("未上传");
			}
			if (data.status == "1") {
				$('td:eq(6)', row)
						.html(
				...
				$('td:eq(6)', row).html(
						"<button class='btn btn-disabled btn-outline btn-search' >已注销</button>");
		...
		function reUpload(taskId) {
			var goUrl = "/claimcar/platformAlternately/platformTaskReloadInit.do?taskId="
					+ taskId;
		...
		function search() {
			...
			var registNo = $("*[name='platformQueryVo.registNo']").val().trim();
			if((isBlank(claimSeqNo))&& (isBlank(bussNo))&& (isBlank(registNo))){
				layer.alert("案件号至少录入一项");
				return false;
			}
			var ajaxList = new AjaxList("#resultDataTable");
			ajaxList.targetUrl = "/claimcar/platformAlternately/searchTask.do";// + $("#form").serialize();

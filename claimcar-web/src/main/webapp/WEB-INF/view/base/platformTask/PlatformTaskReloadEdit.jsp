	...
	<div class="page_wrap">
		<div class="table_cont">
			<div class="table_wrap">
				...
				<div class="table_cont">
					<div class="row mb-3 cl">
						...
						<div class="formControls col-3">${taskVo.requestName}</div>
						<label class="form-label col-1 text-c">归属机构:</label>
						<div class="formControls col-3">
							<app:codetrans codeType="ComCode" codeCode="${taskVo.comCode}" />
						</div>
						<label class="form-label col-1 text-c">上传状态:</label>
						<div class="formControls col-3">
							<c:if test="${taskVo.status eq '1'}">上传成功</c:if>
							<c:if test="${taskVo.status eq '0'}">上传失败</c:if>
						</div>
					</div>
					<div class="row mb-3 cl">
						<label class="form-label col-1 text-c">理赔编码:</label>
						<div class="formControls col-3">${taskVo.claimSeqNo}</div>
						<label class="form-label col-1 text-c">业务号:</label>
						<div class="formControls col-3">${taskVo.bussNo}</div>
					</div>
					<div class="row mb-3 cl">
						<label class="form-label col-1 text-c">错误原因:</label>
						<div class="formControls col-11">${taskVo.remark}</div>
					</div>
				</div>
			</div>
			<div class="table_wrap">
			...
			<input style="margin-left: 40%;" class="btn btn-primary btn-kk"
				type="button" onclick="reUpload('${taskVo.id}','${logId}')" value="补传" /> <input
				class="btn btn-primary btn-kk" type="button" onclick="closeLayer()"
				value="返回" />
		</div>
	</div>
	<script type="text/javascript">
		function reUpload(taskId,logId){
			var url = "/claimcar/platformAlternately/platformTaskReload.do";
			var loadIdx = layer.load(0, {time: 30*2000},{shade: 0});
			$.ajax({
				url : url, // 后台处理程序
				type : 'post', // 数据发送方式
				dataType : 'json', // 接受数据格式
				data : {"logId" : logId,"taskId" : taskId}, // 要传递的数据
				async : true,

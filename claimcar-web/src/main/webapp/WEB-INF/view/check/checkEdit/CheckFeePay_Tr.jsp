...
<c:forEach var="chargeFeeVo" items="${lossChargeVos}" varStatus="status">
	<tr>
		...
		<td>
			<input type="text" class="input-text "  name="lossChargeVos[${status.index + size  }].chargeName" value="${chargeFeeVo.chargeName }" readonly="readonly"/>
		</td>
			<td>
				<input type="text" class="input-text "  name="lossChargeVos[${status.index + size  }].chargeFee" 
				datatype="amount" onblur="calSumChargeFee()" value="${chargeFeeVo.chargeFee }"/>
			</td>

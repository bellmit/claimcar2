		...
		<div class="formtable mt-10">
			<div class="row cl">
				...
				<div class="form_input col-2">			
					<input type="text" class="input-text" name="prpdcheckBankVo.AccountName" value="${prpdcheckBank.accountName}"  id="accountname" onchange="checkSpecialCharactor(this)" />
				</div>
				<span class="c-red col-1">*</span> 
				<label class="form_label col-2">收款方账号：</label>
				<div class="form_input col-2 ">
					<input type="text" class="input-text"
						name="prpdcheckBankVo.AccountNo" value="${prpdcheckBank.accountNo}"  id="account" onchange="checkSpecialByAccountNo(this)" />
				</div>
				<span class="c-red col-1">*</span>
			</div>
			...
			<div class="row cl">
				...
				<div class="form_input col-2">
					<input type="text" class="input-text"
						name="prpdcheckBankVo.CertifyNo" value="${prpdcheckBank.certifyNo}" id="certifyNo" onchange="checkCertifyNo()"/>
				</div>
				<span class="c-red col-1">*</span>
...
<script type = "text/javascript">
 ...
 $(function(){
	...
	$("#accountname").val(parent.$("#checkBankAccountName").val());
	$("#account").val(parent.$("#checkBankAccountNo").val());
	$("#accountcellphone").val(parent.$("#Mobile").val());
	$("#intermBankProvCity").val(parent.$("#City").val());
	$("#BankName").val(parent.$("#BankCodeA").val());
	$("#BankName option:selected").text(parent.$("#checkBankName").val());
 ...
 function layerShowBankNum() {//弹出检索行号页面
		...
		City = $("#checkBankProvCity_lv2  option:selected").text();
		Bname = $("#BankName  option:selected").text();
		BQuery = $("#BankQueryText").val();
		$("#Province").val($("#checkBankProvCity_lv1  option:selected").val());
		$("#City").val($("#checkBankProvCity_lv2  option:selected").val());

		if (Prov == "" || Bname == "" || City == "") {
			layer.msg("请完善开户省市及银行信息！");

		} else {
			url = "/claimcar/checkagency/findBankNoQueryList";
			if (index2 == null) {
				index2 = layer.open({
					type : 2,
					title : '检索行号',
...
function layerHiddenBank() {//银行信息界面点击“保存”关闭		
	...
	if (accountname == null || accountname == "") {
		...
		if ( $("#BankOutlets").val() == null|| $("#BankOutlets").val() == ""
			...
			parent.$("#checkBankAccountName").val($("#accountname").val());
			parent.$("#checkBankAccountNo").val($("#account").val());
			parent.$("#Mobile").val($("#accountcellphone").val());
			parent.$("#City").val($("#checkBankProvCity").val());
			parent.$("#Provincep").val($("#Province").val());
			parent.$("#BankCodeA").val($("#BankName").val());
			parent.$("#checkBankName").val($("#BankName option:selected").text());
			...
			// 加判断
			if(parent.$("#checkBankAccountName").val() != null && parent.$("#checkBankAccountNo").val() != ""
					&& parent.$("#checkBankName").val() != null){
				layer.msg("保存成功！");
				var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
...
function layerShowBankNum(title, id) {//弹出检索行号页面
	...
	BQuery = $("#BankQueryText").val();
	$("#Province").val(
			$("#checkBankProvCity_lv1  option:selected").val());
	$("#City").val($("#checkBankProvCity_lv2  option:selected").val());
...
function checkSpecialCharactor(element){
 ...
 function checkSpecialChar(str){
  ...
  function cleanBankNum(){//切换归属地或银行后，清空原来的行号信息，要求重新检索
	...
	$("#checkBankProvCity_lv1").change(function(){
		cleanBankNum();
	});
	$("#checkBankProvCity_lv2").change(function(){
		cleanBankNum();
	});
	...
	function checkCertifyNo(){
		 var certifyNo = $("[name='prpdcheckBankVo.CertifyNo']").val();

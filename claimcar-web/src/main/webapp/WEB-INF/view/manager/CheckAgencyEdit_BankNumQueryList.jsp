...
<script type="text/javascript">
	...
	function initQueryView(){
		provN = parent.$("#checkBankProvCity_lv1  option:selected").text();
		cityN = parent.$("#checkBankProvCity_lv2  option:selected").text();
		bname = parent.$("#BankName  option:selected").text();
		bQuery = parent.$("#BankQueryText").val();
		provCode = parent.$("#checkBankProvCity_lv1  option:selected").val();
		cityCode = parent.$("#checkBankProvCity_lv2  option:selected").val();
	...
	function getBankNumTab(bNumber,bOutlets){//点击选中行号信息行赋值
		...
		parent.$("[name='prpdcheckBankVo.BankOutlets']").val(bOutlets);	
		parent.$("[name='prpdcheckBankVo.BankNumber']").val(bNumber);
